...
| `MD_CURATE` | `false` | 残す価値のある会話だけを配信する（下記） |
| `MD_CLIP_PHRASES` | 下記の既定 | 無条件で保存する発話トリガー |
| `MD_LLM_PROVIDER` | `gemini` | `gemini` または `claude` |
| `MD_LLM_MODEL` | `gemini-3.6-flash` | 判定・整形に使うモデル |
| `GEMINI_API_KEY` | なし | `gemini` を使う場合に必要 |
| `ANTHROPIC_API_KEY` | なし | `claude` を使う場合に必要 |
| `MD_REFINE_TRANSCRIPT` | `false` | 文字起こしを整形する（下記） |
| `MD_REFINE_MODEL` | `MD_LLM_MODEL` | 整形だけ別モデルにしたい場合 |
...
## アイデアを取りこぼさない（キュレーション）

このパイプラインの当初の形は「全部ためて後で探す」でした。コミュニティの利用実績は
逆を示しています。**最も使われている Omi プラグインは、発話トリガーで指定した区間**
**だけ**を記録し、他の会話をすべて無視するもので、その課金額は2位の5倍です。

求められているのは完全なアーカイブではなく、**今言ったことを失わないこと**でした。

そこで配信先に届く経路を2つにしています。

### 経路1: 発話トリガー（モデル呼び出しなし）

会話中に「クリップ」と言うだけで、その会話は無条件で保存されます。意図的につけた印を
モデルが後から否定することはありません。呼び出しが発生しないので費用もゼロです。
...
トリガーのない会話は LLM が一度だけ読み、次のどれかを含むかを判定します。

- 新しいアイデア、企画の種、思いつき
- 決定事項、約束、次にやること
- 相手から得た事実（予算・決裁の流れ・時期・人と役割・制約）
- 後で思い出せないと困る気づき

**含まないと判定された会話は配信されません。** 含む場合は、拾った項目が
「拾ったアイデア・気づき」として Markdown と Notion の先頭に入ります。

```ini
MD_CURATE=true
MD_LLM_PROVIDER=gemini
MD_LLM_MODEL=gemini-3.6-flash
GEMINI_API_KEY=...
```
...
- **推測で補いません。** プロンプトで要約・推測・言い換えを禁じています。会話で語られた
  ことだけを拾い、無ければ空で返します
- **判定に失敗した会話は残します。** キュレーションの失敗が、何かを失う理由になっては
  いけません。判定できなければ保持側に倒します
- **判定済みは再判定しません。** 捨てた会話も配信済みとして記録するので、次回の実行で
  同じ判定に二度払うことはありません
- **設定ミスは1回で止まります。** キー未設定などは会話ごとに繰り返さず、最初の失敗で
  キュレーションを止めて**全件を配信します**（黙って捨てません）

### 費用の目安

判定は会話1件につき1回の呼び出しです。1時間の会話でおよそ1〜1.5万トークン。

| モデル | 入力/出力 (per 1M) | 1時間の会話あたり |
|---|---|---|
| `gemini-3.6-flash`（既定） | $1.50 / $7.50 | 約 $0.05 |
| `gemini-3.5-flash-lite` | $0.30 / $2.50 | 約 $0.01 |
| `claude-opus-5` | $5.00 / $25.00 | 約 $0.2 |

出力は判定結果だけなので短く、入力が支配的です。既定の Flash で1日3時間なら月 $5 程度。

## 文字起こしの整形（任意）

Omi の文字起こしは自動認識のため、無音区間から拾われた断片や別言語として誤認識された
挿入、話者不明の発言が混ざります。これを LLM に通して**読める記録**に整えられます。

```ini
MD_REFINE_TRANSCRIPT=true
MD_REFINE_MODEL=            # 空なら MD_LLM_MODEL と同じモデル
```
...
- **内容を足しません。** 聞き取れていない箇所は `[不明瞭]` と記されます
- **失敗しても配信は止まりません。** 原文のまま配信されます

キュレーションと違い、整形は文字起こし全体を入力にするので費用が一桁上がります。
両方を有効にすると会話1件につき2回の呼び出しになります。
